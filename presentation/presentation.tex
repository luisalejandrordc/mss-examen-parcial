...
\begin{frame}{Pregunta 4: Validación Estadística — Prueba de Medias}
	\begin{block}{Contexto Operativo}
		...
		es \textbf{imparcial} (no centrado hacia ningún extremo del intervalo).
	\end{block}

	\begin{alertblock}{Marco Teórico}
		Un conjunto $r_i \sim U(0,1)$ tiene valor esperado teórico
		$E[r] = 0.5$ (la media de la distribución uniforme continua).
		Si el generador estuviera sesgado, los números tenderían sistemáticamente
		hacia 0 o hacia 1, distorsionando todas las variables aleatorias derivadas
		de ellos.\\[6pt]
		La \textbf{Prueba de Medias} compara el promedio muestral $\bar{r}$ de los $n$ números
		generados contra el valor teórico esperado $\mu = 0.5$, con un margen
		de tolerancia estadística controlado por el nivel de significancia $\alpha$.
		\begin{block}{Datos utilizados}
			\small
			100 números pseudoaleatorios generados mediante ACL con parámetros
			$X_0 = 83$, $a = 141$, $c = 53$, $m = 2^{10} = 1024$
			(cumplen condiciones de período máximo $N = 1024$).
		\end{block}
	\end{alertblock}
\end{frame}
...
\begin{frame}{Pregunta 4: Validación Estadística — Prueba de Medias}
	...
	\begin{block}{Formulación de Hipótesis}
		\[
			H_0: \mu_r = 0.5 \qquad \text{vs} \qquad H_1: \mu_r \neq 0.5
		\]
	\end{block}

	\vspace{0.4em}
	\textbf{¿Por qué $\mu = 0.5$?}
	La distribución $U(0,1)$ tiene densidad constante $f(x)=1$ en $(0,1)$.
	Su media teórica es $E[X] = \int_0^1 x\,dx = \tfrac{1}{2} = 0.5$.
	Verificar $H_0$ es verificar que los $r_i$ se comportan como si fueran
	uniformes.
	...
	Por el \textbf{Teorema Central del Límite}, para $n$ grande,
	$\bar{r} \approx \mathcal{N}\!\left(0.5,\;\frac{\sigma^2}{n}\right)$
	con $\sigma^2 = \frac{1}{12}$ (varianza de $U(0,1)$).
	Esto justifica el uso de la distribución normal estándar para construir
	el intervalo de aceptación.

\end{frame}

\begin{frame}{Límites de Aceptación — ¿Por qué esa fórmula?}

	Por el TCL, la distribución muestral de $\bar{r}$ es:
	\[
		\bar{r} \;\sim\; \mathcal{N}\!\left(\mu,\; \frac{\sigma^2}{n}\right)
		\;=\; \mathcal{N}\!\left(0.5,\; \frac{1/12}{n}\right)
		\;=\; \mathcal{N}\!\left(0.5,\; \frac{1}{12n}\right)
	\]

	Un intervalo de confianza bilateral al $(1-\alpha)\times100\%$ para $\mu$:
	\[
		P\!\left(\mu - z_{\alpha/2}\frac{\sigma}{\sqrt{n}}
		\;\leq\; \bar{r}
		\;\leq\; \mu + z_{\alpha/2}\frac{\sigma}{\sqrt{n}}\right)
		= 1 - \alpha
	\]

	Sustituyendo $\mu = 0.5$ y $\sigma = \sqrt{1/12}$:
	...
	\vspace{0.3em}
	\begin{block}{Interpretación del factor $1/\sqrt{12n}$}
		\small
		Es el \textbf{error estándar} de $\bar{r}$: combina la dispersión
		intrínseca de $U(0,1)$ — capturada en $\sigma = 1/\sqrt{12}$ — con la
		reducción de incertidumbre al promediar $n$ observaciones
		(factor $1/\sqrt{n}$). A mayor $n$, el intervalo se estrecha
		y la prueba es más exigente.
	\end{block}
