...
\begin{frame}{Pregunta 4: Validación Estadística — Prueba de Medias}
	\begin{emptyblock}
		...
		\]
		% Un intervalo de confianza bilateral al $(1-\alpha)\times 100\%$
		% para $\mu$ toma la forma $\bar{r} \pm z_{\alpha/2}\,\sigma/\sqrt{n}$.
		% Sustituyendo $\mu = 0.5$ y $\sigma = 1/\sqrt{12}$:
		Bajo la hipótesis de un generador imparcial ($\mu = 0.5$), los \textbf{límites de aceptación} para $\bar{r}$ con un nivel de confianza $(1-\alpha)\cdot 100\%$ se definen como:
		\[
			\boxed{
...
\begin{frame}{Aplicación Numérica}

	\textbf{Parámetros:} $n = 100$, \;$\alpha = 0.05$, \;$z_{0.025} = 1.96$

	\vspace{0.5em}
	\textbf{1. Promedio muestral:}
	\[
		\bar{r} \;=\; \frac{1}{100}\sum_{i=1}^{100} r_i
		\;=\; \frac{46.5748}{100} \;=\; \mathbf{0.4657}
	\]
	...
	\begin{block}{Verificación}
		\[
			...
			0.4434 \;\leq\; 0.4657 \;\leq\; 0.5566 \quad \checkmark
		\]
	\end{block}

\end{frame}

\begin{frame}{Conclusión — Prueba de Medias}
	...
	\begin{center}
		...
		\begin{tabular}{lc}
			...
			Promedio muestral $\bar{r}$     & 0.4657         \\
			Límite inferior $LI$            & 0.4434         \\
			Límite superior $LS$            & 0.5566         \\
			$\bar{r} \in [LI,\,LS]$?        & Sí \checkmark  \\
			\bottomrule
		\end{tabular}
	\end{center}
	...
	\begin{alertblock}{Conclusión estadística}
		Como $\bar{r} = 0.4657$ se encuentra dentro del intervalo de aceptación
		$[0.4434,\;0.5566]$, \textbf{no se rechaza $H_0$} con un nivel de
		confianza del 95\%.
		Se concluye que el conjunto de 100 números pseudoaleatorios
		\textbf{tiene un valor esperado estadísticamente compatible con 0.5},
		validando la imparcialidad del generador ACL utilizado.
	\end{alertblock}

	\vspace{0.3em}
	\small\textit{Nota:} Al recibir los datos del profesor, reemplazar
	$\bar{r}$, $n$ y $\alpha$; la estructura del argumento y las fórmulas
	son idénticas.
